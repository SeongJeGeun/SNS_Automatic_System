...
class CEORecommendationWorker:
    ...
    def run(self, task: AgentTask) -> AgentResult:
        ...
        user = (
            "현재 런타임 상태를 보고 다음 회차 운영 판단을 내려주세요.\n"
        ...
        if result and result.ok:
            return AgentResult(
                ...
                data={"recommendation": result.content, "llm": result.__dict__},
            )
        fallback = (
            "Instagram은 쿨다운 여부를 우선 확인하고, 제한 중이면 Threads만 발행하세요.\n"
            "다음 회차는 최근 주제와 다른 감정 축을 선택하세요.\n"
        ...
        return AgentResult(
            ...
            data={"recommendation": fallback, "error": result.error if result else "router unavailable"},
        )
